...
const AccountPage = () => {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("display_name, phone, username, avatar_url").eq("id", user.id).single()
      .then(({ data }) => {
        if (data) {
          setProfile(data);
          setForm({ display_name: data.display_name || "", phone: data.phone || "", username: data.username || "" });
        }
      });
  ...
  if (loading) {
    return (<div className="min-h-screen flex items-center justify-center bg-background"><div className="animate-spin rounded-full h-8 w-8 border-2 border-primary border-t-transparent" /></div>);
  }
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="bg-gradient-to-br from-primary via-primary to-chad-blue p-6 pt-8 text-primary-foreground relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-4">
          <button className="relative w-18 h-18 rounded-2xl bg-primary-foreground/20 flex items-center justify-center overflow-hidden group shadow-lg" style={{ width: '72px', height: '72px' }} onClick={() => fileInputRef.current?.click()} disabled={uploading}>
            {profile?.avatar_url ? (<img src={profile.avatar_url} alt="Avatar" className="w-full h-full object-cover" />) : (<User className="h-8 w-8" />)}
            ...
            {uploading && (<div className="absolute inset-0 bg-foreground/50 flex items-center justify-center rounded-2xl"><div className="animate-spin rounded-full h-5 w-5 border-2 border-primary-foreground border-t-transparent" /></div>)}
          </button>
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-extrabold truncate">{displayName}</h2>
            <p className="text-sm opacity-80 flex items-center gap-1.5 mt-0.5"><Phone className="h-3 w-3 shrink-0" /><span className="truncate">{displayPhone}</span></p>
          </div>
          {!editing && (<button onClick={() => setEditing(true)} className="bg-primary-foreground/20 rounded-xl p-2.5 hover:bg-primary-foreground/30 transition-colors"><Pencil className="h-4 w-4" /></button>)}
        </div>
      </div>
      ...
      {editing && (
        <div className="p-4 bg-card border-b space-y-3 animate-fade-in">
          ...
          <div className="space-y-2.5">
            <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t("account.displayName")}</label><Input value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} className="h-11 rounded-xl bg-muted/50 border-0" /></div>
            <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t("account.phone")}</label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="h-11 rounded-xl bg-muted/50 border-0" /></div>
            <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t("account.username")}</label><Input value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="@username" className="h-11 rounded-xl bg-muted/50 border-0" /></div>
          </div>
          <div className="flex gap-2 pt-1">
            <Button onClick={handleSave} disabled={saving} className="flex-1 gap-1.5 h-11 rounded-xl font-bold"><Check className="h-4 w-4" />{saving ? "..." : t("account.save")}</Button>
            <Button variant="outline" className="h-11 rounded-xl" onClick={() => { setEditing(false); if (profile) setForm({ display_name: profile.display_name || "", phone: profile.phone, username: profile.username || "" }); }}><X className="h-4 w-4" /></Button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3 p-4">
        {[
          { icon: FileText, value: stats.totalListings, label: t("account.ads"), color: "text-primary" },
          { icon: Eye, value: stats.totalViews, label: t("account.views"), color: "text-chad-yellow" },
          { icon: Heart, value: stats.totalFavorites, label: t("account.favs"), color: "text-chad-red" },
        ].map((stat) => (
      ...
      <div className="px-4 space-y-2">
        {[
          { label: t("account.myListings"), icon: FileText, path: "/mes-annonces" },
          { label: t("account.myFavorites"), icon: Heart, path: "/favoris" },
          ...(isAdmin ? [{ label: t("account.admin"), icon: ShieldCheck, path: "/admin" }] : []),
        ].map((item) => (
          <button key={item.path} onClick={() => navigate(item.path)} className="w-full flex items-center gap-3 p-4 bg-card rounded-2xl shadow-card hover:shadow-card-hover transition-all active:scale-[0.98]">
            <div className="p-2 rounded-xl bg-accent"><item.icon className="h-4 w-4 text-accent-foreground" /></div>
        ...
        <div className="pt-4">
          <Button variant="ghost" className="w-full justify-start text-destructive hover:text-destructive hover:bg-destructive/5 rounded-2xl h-12" onClick={async () => { await signOut(); navigate("/"); }}>
            <LogOut className="h-5 w-5 mr-2" />{t("account.logout")}
          </Button>
        </div>
      </div>
